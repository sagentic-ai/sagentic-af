...
describe("Session", () => {
  ...
  describe("SessionBudgetHandler", () => {
    ...
    test("Session without budget handler throws error when budget exceeded", async () => {
      ...
      await expect(session.invokeModel(
        agent,
        models[BuiltinModel.GPT35Turbo],
        [
          { role: MessageRole.System, content: "You are a helpful assistant." },
    ...
    test("Session with budget handler that increases budget allows continuation", async () => {
      const budgetHandler = jest.fn<Promise<number>, [number, number, Message[], Session]>(async (totalCost, budget, nextMessages, session) => {
    ...
    test("Budget handler can examine next messages to make decisions", async () => {
      const budgetHandler = jest.fn<Promise<number>, [number, number, Message[], Session]>(async (totalCost, budget, nextMessages, session) => {
        ...
        const hasImportantContent = nextMessages.some(msg => {
          if (typeof msg.content === 'string') {
            return msg.content.includes("URGENT") || msg.content.includes("CRITICAL");
          }
          return false;
        });
      ...
      await session.invokeModel(
        ...
        [
          { role: MessageRole.System, content: "You are a helpful assistant." },
          { role: MessageRole.User, content: "URGENT: This is a critical request!" }
        ]
      );
